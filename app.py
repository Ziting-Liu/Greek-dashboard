...
with tab_vol:
    ...
    USING_REAL_IV_HISTORY_THRESHOLD = 20  # days of real snapshots before we trust them over the proxy
    ...
    if current_atm_iv is not None:
        ...
        real_iv_rank, real_iv_percentile = data_fetch.iv_rank_and_percentile(history, current_atm_iv)
    else:
        history, real_iv_rank, real_iv_percentile = pd.DataFrame(), None, None

    using_proxy = len(history) < USING_REAL_IV_HISTORY_THRESHOLD
    if using_proxy and current_atm_iv is not None:
        iv_rank, iv_percentile = data_fetch.realized_vol_rank_and_percentile(tkr, current_atm_iv)
    else:
        iv_rank, iv_percentile = real_iv_rank, real_iv_percentile

    col1, col2, col3 = st.columns(3)
    col1.metric("ATM Implied Vol (nearest expiry)", f"{current_atm_iv:.1%}" if current_atm_iv else "n/a")

    rank_label = "IV Rank (proxy)" if using_proxy else "IV Rank"
    pct_label = "IV Percentile (proxy)" if using_proxy else "IV Percentile"
    if iv_rank is not None:
        col2.metric(rank_label, f"{iv_rank:.0f}")
        col3.metric(pct_label, f"{iv_percentile:.0f}")
    else:
        col2.metric(rank_label, "n/a")
        col3.metric(pct_label, "n/a")

    if using_proxy:
        st.info(
            f"Yahoo Finance doesn't provide historical implied vol, so real IV Rank/Percentile need this "
            f"app to build its own history one snapshot per day -- it has {len(history)}/{USING_REAL_IV_HISTORY_THRESHOLD} "
            f"day(s) recorded for {symbol} so far. Until then, the numbers above are a **proxy** computed "
            f"against a year of realized (historical) volatility instead of real implied vol -- directionally "
            f"useful, but implied and realized vol can diverge meaningfully, so don't treat this as exact. "
            f"It'll switch to real IV-based rank/percentile automatically once enough snapshots build up."
        )
